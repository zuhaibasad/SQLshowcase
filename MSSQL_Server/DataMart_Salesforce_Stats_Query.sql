-- this a MS-SQL query
-- it is very complex due to various data sources are involved
-- which are difficult to integrate and some datasource are old
-- and some are very new. 
-- This query is being used by the company to monitor a very critical strategy.


Select corpavg.*, AVG(corpavg.CycleCPC) OVER (Partition By corpavg.corp_id, corpavg.cyclenumber) as CorpCPCAvg,
...
From (Select cycle.*, 
 ...
 From (Select grp.*,SUM(grp.spend) OVER (Partition By grp.corp_id, grp.IDLocationName, grp.cyclenumber Order By grp.cyclenumber ASC) as CycleSpend,
        ...
        From (Select master.corp_id, MAX(master.corp_name) as corp_name, master.full_client_id, MAX(master.day) as stats_day, MAX(master.begintime_session) as sess_day,master.full_date, MAX(master.weeknumber) as weeknumber, MAX(master.year) as year, 
                       ...
                       MAX(master.nextbillingdate) as nextbillingdate, MAX(master.previousbillingdate) as previousbillingdate, MAX(master.ClosedDate) as ClosedDate, 
                       MAX(master.CaseNumber) as CaseNumber, MAX(master.product_status) as product_status, MAX(master.RecordType) as RecordType
               ...
               From (Select DISTINCT stats_sess_client.*, DATEADD(day, +1, stats_sess_client.previousbillingdate) as first_cycle_day, 
                                        ...
                                        baltab.balance, baltab.refresheded_at, SF.ClosedDate, SF.CaseNumber, cp.product_status, SF.RecordType
                     From (
                              ...
                              FULL OUTER JOIN (Select c2.client_id as cid, c2.location_name, c2.nextbillingdate, DATEADD(month, -1, c2.nextbillingdate) as previousbillingdate,
                                   ...
                                   (Select client_id, STRING_AGG(product, ', ') as product,
                                    ...
                                    Where product LIKE 'ATV_SEM' OR product LIKE 'SEM'
                                    Group By client_id) cp ON stats_sess_client.full_client_id=cp.client_id
                                   
                                   LEFT JOIN 
                                   
                                   (Select round(cast(pac.balance_micros as bigint)/1000000,2) as balance, pac.refresheded_at, cast(pacrm.domain_key as bigint) as acquisio_client_id, pacrm.created, pacrm.start_date 
                                   ...
                                   LEFT JOIN (Select b.*
                                              From (Select Convert(bigint, Client_ID__c) as SF_ClientID, MAX(ClosedDate) as ClosedDate
                                                    From dbo.salesforcecase 
                                                    Where ClosedDate> '01/01/2020' AND ISNUMERIC(Client_ID__c)=1 AND Name LIKE 'YBN Request'
                                                    Group By Client_ID__c) a JOIN (Select Convert(bigint, Client_ID__c) as SF_ClientID, ClosedDate, Name as RecordType, CaseNumber
                                                                                   From dbo.salesforcecase
                                                                                   Where ClosedDate> '01/01/2020' AND ISNUMERIC(Client_ID__c)=1) b ON a.SF_ClientID=b.SF_ClientID and a.ClosedDate=b.ClosedDate) 
                                    SF on stats_sess_client.full_client_id=SF.SF_ClientID AND stats_sess_client.full_date=convert(date,SF.ClosedDate)
                                    LEFT JOIN acquisio_budgetbalance abb ON stats_sess_client.full_client_id=abb.clientid and stats_sess_client.stats_date=abb.DataExtractedAt) master
         
          Where master.full_client_id IS NOT NULL and master.corp_id IS NOT NULL
